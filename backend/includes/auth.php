...
function recordFail(string $ip): void {
    $f = rlFile($ip);
    $d = file_exists($f) ? json_decode(file_get_contents($f), true) : ['attempts' => 0, 'first' => time()];
    $d['attempts']++;
    file_put_contents($f, json_encode($d), LOCK_EX);
}
function clearRL(string $ip): void { $f = rlFile($ip); if (file_exists($f)) unlink($f); }

function attemptLogin(string $username, string $password): array {
    $ip = $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0';
    if (!checkRateLimit($ip)) return ['ok' => false, 'error' => 'Too many attempts. Wait 5 minutes.'];
    if (strlen($username) > 64 || strlen($password) > 128) return ['ok' => false, 'error' => 'Invalid credentials.'];

    $pdo  = getDB();
    $stmt = $pdo->prepare('SELECT id, username, password, display_name, is_active FROM admin_users WHERE username = ? LIMIT 1');
    ...
    return ['ok' => true, 'name' => $admin['display_name'], 'username' => $admin['username']];
}